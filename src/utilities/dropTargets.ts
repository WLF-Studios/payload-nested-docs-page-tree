...
export function buildInsertDropTargets(docs: PageTreeDoc[]): PageTreeInsertDropTarget[] {
  const rootDocs = docs.filter((doc) => doc.__pageTreeParentID === null)

  if (rootDocs.length === 0) {
    return []
  }

  return Array.from({ length: rootDocs.length + 1 }, (_, index) => {
    const referenceDoc = rootDocs[index] ?? rootDocs[rootDocs.length - 1]
...
export function getOrderPlacementFromDropTarget(args: {
  ...
  const siblingDocs = docs.filter(
    (doc) => doc.__pageTreeParentID === parentID && !movingSubtreeIDs.has(doc.__pageTreeID),
  )
  const beforeSiblings = siblingDocs.slice(0, insertIndex)
  const afterSiblings = siblingDocs.slice(insertIndex)
